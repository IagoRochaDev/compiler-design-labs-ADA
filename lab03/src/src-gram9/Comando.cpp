...
static void extrai_fim_if(No_arv_parse* no, ComandoIf* if_node) {
  if (no == NULL) return;
  if (no->regra == 49) { // FimIf -> END IF PONTO_VIRGULA
    return;
  }
  if (no->regra == 50) { // FimIf -> ELSIF Expressao THEN ListaComandos FimIf
  ...
  if (no->regra == 51) { // FimIf -> ELSE ListaComandos END IF PONTO_VIRGULA
...
vector<Comando*> Comando::extrai_lista_comandos(No_arv_parse* no) {
  if (no == NULL || no->regra == 11) return vector<Comando*>(); // ListaComandos ->
...
Comando* Comando::extrai_comando(No_arv_parse* no) {
  ...
  if (no->regra == 48) { // ComandoIf -> IF Expressao THEN ListaComandos FimIf
    ...
    return res;
  } else if (no->regra == 52) { // ComandoWhile -> WHILE Expressao LOOP ListaComandos END LOOP PONTO_VIRGULA
    ComandoWhile* res = new ComandoWhile();
    res->condicao = Expressao::extrai_expressao(no->filhos[1]);
    res->comandos = extrai_lista_comandos(no->filhos[3]);
    return res;
  } else if (no->regra == 58) { // ComandoReturn -> RETURN Expressao PONTO_VIRGULA
    ComandoRetorno* res = new ComandoRetorno();
    res->expressao = Expressao::extrai_expressao(no->filhos[1]);
    return res;
  } else if (no->regra == 59) { // ComandoReturn -> RETURN PONTO_VIRGULA
    ComandoRetorno* res = new ComandoRetorno();
    res->expressao = NULL;
    return res;
  } else if (no->regra == 60) { // ComandoAtribuicao -> Acesso ATRIBUICAO Expressao PONTO_VIRGULA
