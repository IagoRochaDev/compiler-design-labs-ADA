...
ValorLiteral ExpressaoSubtracao::avalia(TabelaSimbolos* memoria) {
  ValorLiteral left = esquerda->avalia(memoria);
  ValorLiteral right = direita->avalia(memoria);
  if (left.tipo == NULL || right.tipo == NULL) {
    throw runtime_error("Tipo invalido em operacao de subtracao");
  }
  if (left.tipo->valor == Tipo::BOOL || right.tipo->valor == Tipo::BOOL) {
    throw runtime_error("Operacao de subtracao invalida para tipo booleano");
  }
